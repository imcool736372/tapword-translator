...
async function setupTooltipSpacingPreview(): Promise<void> {
  ...
  if (!stage || !paragraph || !anchor1 || !anchor || !tooltip1 || !tooltip || !gapInput || !offsetInput || !fontPresetSelect || !autoAdjustHeightInput) {
    return
  }
  ...
  const updatePreview = () => {
    ...
    const resolved = translationFontSizeModule.resolveTranslationFontSize(
      fontPresetSelect.value as types.TranslationFontSizePreset
    )
